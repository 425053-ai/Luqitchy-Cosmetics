...
const product = {
  ...
  scents: [
    { id: "mulberry", name: "Mulberry", image: "/images/body-lotion-splash-mulberry.jpeg" },
    { id: "sugar-drop", name: "Sugar Drop", image: "/images/body-lotion-splash-sugardrop.jpeg" },
  ],
}
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await emailjs.send("service_cyg3pcs", "template_nn2n23j", {
        ...
        product_variant: selectedScent.name,
    ...
    return (
      <div className="min-h-screen flex flex-col justify-center items-center p-4 bg-background text-center space-y-6">
        ...
        <div className="bg-white p-4 rounded-lg shadow-md max-w-md w-full text-left">
          ...
          <p><strong>النكهة:</strong> {selectedScent.name}</p>
    ...
    <div className="min-h-screen bg-background p-4">
      ...
      <div className="max-w-4xl mx-auto grid lg:grid-cols-2 gap-12">
        <div className="relative aspect-[3/4] rounded-3xl overflow-hidden shadow-2xl shadow-primary/30">
          <Image src={selectedScent.image} alt={selectedScent.name} fill className="object-cover" />
          <div className="flex gap-2 mt-2 justify-center">
            {product.scents.map((s) => (
              <button key={s.id} onClick={() => setSelectedScent(s)} className={`px-3 py-1 border rounded ${s.id === selectedScent.id ? "bg-accent text-white" : ""}`}>
                {s.name}
              </button>
            ))}
          </div>
        ...
        <div className="space-y-6">
          <h1 className="text-4xl font-bold">{product.name} - {selectedScent.name}</h1>
